...
def run_pipeline(img, depths, min,bright,illuminant,fract,balance):
    ptsR, ptsG, ptsB = find_backscatter_estimation_points(img, depths, fraction=0.01, min_depth_percent=min)
    ...
    illR = estimate_illumination(img[:, :, 0], Br, nmap, n, p=illuminant, max_iters=100, tol=1E-5, f=bright)
    illG = estimate_illumination(img[:, :, 1], Bg, nmap, n, p=illuminant, max_iters=100, tol=1E-5, f=bright)
    illB = estimate_illumination(img[:, :, 2], Bb, nmap, n, p=illuminant, max_iters=100, tol=1E-5, f=bright)
    ill = np.stack([illR, illG, illB], axis=2)

    beta_D_r, _ = estimate_wideband_attentuation(depths, illR)
    refined_beta_D_r, coefsR = refine_wideband_attentuation(depths, illR, beta_D_r, radius_fraction=fract, l=balance)
    beta_D_g, _ = estimate_wideband_attentuation(depths, illG)
    refined_beta_D_g, coefsG = refine_wideband_attentuation(depths, illG, beta_D_g, radius_fraction=fract, l=balance)
    beta_D_b, _ = estimate_wideband_attentuation(depths, illB)
    refined_beta_D_b, coefsB = refine_wideband_attentuation(depths, illB, beta_D_b, radius_fraction=fract, l=balance)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output', default='stereo_out.png', help='Output filename')
